//Процесс заряжания палочек. развитие скилов зависит от уровня. Если навык не больше равен значению в итеме - процесс неудачен в 90% случаев минимум.
...
void pt_ChargeItemNAMC(object oActivator, object oItem)
{
    ...
    if(pResponse(FloatToInt(IntToFloat(iChanceToCreate)*fK)))
        ...
        int iAncorBaseType=GetBaseItemType(oItem); //Заряжаем только тот итем что подходит по типу
                    ...
                    if(GetTag(oActItem)!=sCurRawTag)
                        ...
                        if(GetIsObjectValid(oChargItem))
                            ...
                            if(pResponse(FloatToInt(IntToFloat(iChanceToCreate)*fK)))
                                ...
                                {
                                FloatingTextStringOnCreature(StringToRGBString("Ваших навыков оказалось недостаточно зарядки предмета.", "700"), oActivator, FALSE);
                                };
