...
{ // PURPOSE: To return the skin object and if need be create it
    object oSkin=GetLocalObject(oCreature,"SKIN_pt_craft_tool_");
...
////    } // find skin
    if (!GetIsObjectValid(oSkin))
    { // create it
        oSkin=GetItemPossessedBy(oCreature,"pt_craft_tool_");
        if (GetIsObjectValid(oSkin))
        { // equip it already
////            DelayCommand(1.0,AssignCommand(oCreature,SKIN_SupportEquipSkin(oSkin)));
            SetLocalObject(oCreature,"SKIN_pt_craft_tool_",oSkin);
            SetDroppableFlag(oSkin,FALSE);
            return oSkin;
        } // equip it already
        else
        { // don't have one
           oSkin=CreateItemOnObject("pt_axcrf_it028",oCreature);
